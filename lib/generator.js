...
TemplateGenerator.prototype.render = function render(mail, cb) {
    ...
    var text_templatePath='';
    if (mail.data.text_template!==undefined)text_templatePath=path.join(this.viewPath, mail.data.text_template + this.extName);
    var viewEngine=this.viewEngine;
    this.viewEngine.renderView(templatePath, mail.data.context, function(err, body) {
        if (err) return cb(err);
        mail.data.html = body;
        if(mail.data.text_template===undefined)cb();
        else{
            viewEngine.renderView(text_templatePath, mail.data.context, function(err, body) {
                if (err) return cb(err);
                mail.data.text = body;
                cb();
            });
